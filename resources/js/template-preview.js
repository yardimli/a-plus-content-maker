...
if (dataElement && previewElement && frameElement) {
    ...
    const modules = JSON.parse(dataElement.textContent || '[]');
    ...
    previewElement.innerHTML = modules.length
        ? `<main class="amazon-preview">${modules.map(renderModule).join('')}</main>`
        : '<div class="builder-empty"><h2>This template has no modules yet</h2></div>';
