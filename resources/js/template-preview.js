...
export function renderTemplateModule(module) {
    const previewImage = (source, enabled = true) => source ? `<img src="${escapeHtml(source)}" style="filter:${enabled ? filterCss(module.image_filters || []) : 'none'}" alt="">` : '<span class="amazon-missing-image"></span>';
    ...
    switch (module.module_type) {
        ...
        case 'light_text_overlay':
            return `<section class="amazon-module amazon-overlay ${module.module_type === 'dark_text_overlay' ? 'dark-copy' : 'light-copy'}">${previewImage(content.background, content.background_apply_filters !== false)}<div class="overlay-copy">${previewHeading(content.headline)}${previewRich(content.body_html)}</div></section>`;
        ...
        case 'single_image_highlights':
            return `<section class="amazon-module amazon-highlights">${previewImage(content.image, content.image_apply_filters !== false)}<div class="highlight-copy">${sections.map((section) => `<article>${previewHeading(section.subheadline, 3)}${previewRich(section.body_html)}</article>`).join('')}</div><aside>${previewHeading(content.highlights_headline, 3)}<ul>${bullets.map((bullet) => `<li>${escapeHtml(bullet.text || '')}</li>`).join('')}</ul></aside></section>`;
        case 'single_image_sidebar':
            return `<section class="amazon-module amazon-sidebar"><figure>${previewImage(content.primary_image, content.primary_image_apply_filters !== false)}${content.image_caption ? `<figcaption>${escapeHtml(content.image_caption)}</figcaption>` : ''}</figure><div>${previewHeading(content.headline)}${previewHeading(content.subheadline, 3)}${previewRich(content.body_html)}${bullets.length ? `<ul>${bullets.map((bullet) => `<li>${escapeHtml(bullet.text || '')}</li>`).join('')}</ul>` : ''}</div><aside>${previewImage(content.sidebar_image, content.sidebar_image_apply_filters !== false)}${previewHeading(content.sidebar_headline, 3)}${previewRich(content.sidebar_body_html)}</aside></section>`;
        case 'single_image_specs_detail':
            return `<section class="amazon-module amazon-spec-detail">${previewHeading(content.headline)}<div class="spec-detail-grid">${previewImage(content.image, content.image_apply_filters !== false)}${sections.map((section) => `<article>${previewHeading(section.headline, 3)}${previewHeading(section.subheadline, 4)}${previewRich(section.body_html)}</article>`).join('')}</div></section>`;
        case 'single_left_image':
            return `<section class="amazon-module amazon-single-image left">${previewImage(content.image, content.image_apply_filters !== false)}<div>${previewHeading(content.headline)}${previewRich(content.body_html)}</div></section>`;
        case 'single_right_image':
            return `<section class="amazon-module amazon-single-image right"><div>${previewHeading(content.headline)}${previewRich(content.body_html)}</div>${previewImage(content.image, content.image_apply_filters !== false)}</section>`;
        ...
        case 'three_images_text':
            return `<section class="amazon-module amazon-feature-columns">${previewHeading(content.headline)}<div class="columns three">${items.map((item) => `<article>${previewImage(item.image, item.image_apply_filters !== false)}${previewHeading(item.headline, 3)}${previewRich(item.body_html)}</article>`).join('')}</div></section>`;
        default:
            return '';
    }
}
