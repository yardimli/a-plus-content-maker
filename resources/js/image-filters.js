...
function overlayFilter(overlay) {
    ...
    svg.innerHTML = `<filter id="${id}" x="0" y="0" width="100%" height="100%" color-interpolation-filters="sRGB"><feImage href="data:image/svg+xml,${encodeURIComponent(art)}" preserveAspectRatio="none" result="overlay"/><feComponentTransfer in="SourceGraphic" result="opaqueImage"><feFuncA type="linear" slope="0" intercept="1"/></feComponentTransfer><feBlend in="overlay" in2="opaqueImage" mode="${blend}" result="blended"/><feComposite in="blended" in2="SourceAlpha" operator="in"/></filter>`;
